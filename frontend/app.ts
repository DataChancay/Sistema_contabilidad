...
type ConsolidadoSummary = {
  total_operaciones_culqi: number;
  conciliados_xafiro: number;
  conciliados_mifact: number;
  no_encontrados: number;
  registros_revisar: number;
  total_conciliado: number;
  total_no_conciliado: number;
};
...
const viewLinks = Array.from(document.querySelectorAll<HTMLAnchorElement>("[data-view-link]"));
...
const exportButtonLabel = document.querySelector<HTMLElement>(".button-label");
...
const consolidadoForm = document.querySelector<HTMLFormElement>("#consolidado-form");
const consolidadoMessage = document.querySelector<HTMLElement>("#consolidado-message");
const consolidadoButton = document.querySelector<HTMLButtonElement>("#consolidado-button");
const consolidadoButtonLabel = document.querySelector<HTMLElement>(".consolidado-button-label");
const consolidadoSummaryPanel = document.querySelector<HTMLElement>("#consolidado-summary");
...
let consolidadoBlob: Blob | null = null;
...
const showMessage = (element: HTMLElement, text: string, kind: "error" | "success"): void => {
  element.textContent = text;
  element.className = `form-message ${kind}`;
  element.hidden = false;
};

const clearMessage = (element: HTMLElement): void => {
  element.hidden = true;
  element.textContent = "";
...
const viewFromHash = (): ViewName => (window.location.hash === "#consolidado" ? "consolidado" : "exportables");
...
const setExportLoading = (loading: boolean): void => {
...
const getSelectedSources = (): Source[] => {
  return sourceInputs
    ...
    .map((input) => {
      if (input.value === "neo") return "neo";
      if (input.value === "culqi") return "culqi";
      if (input.value === "mifact") return "mifact";
      return "xafiro";
    });
};

const getExportEndpoint = (source: Source): string => {
  if (source === "neo") return "/api/neo/export";
  if (source === "culqi") return "/api/culqi/export";
  if (source === "mifact") return "/api/mifact/export";
  return "/api/xafiro/export";
};

const getSourceLabel = (source: Source): string => {
  if (source === "neo") return "Neo";
  if (source === "culqi") return "Culqi";
  if (source === "mifact") return "Mifact";
  return "Xafiro";
};
...
exportForm.addEventListener("submit", async (event) => {
  ...
  try {
    ...
    const exported = results.filter((result): result is PromiseFulfilledResult<Source> => result.status === "fulfilled").map((result) => getSourceLabel(result.value));
    const failed = results.filter((result): result is PromiseRejectedResult => result.status === "rejected").map((result) => (result.reason instanceof Error ? result.reason.message : "Ocurri\u00f3 un error inesperado."));
    if (failed.length > 0) {
      hideExportAnimation();
...
const resetConsolidadoDownload = (): void => {
...
const setConsolidadoLoading = (loading: boolean): void => {
...
const parseSummary = (response: Response): ConsolidadoSummary | null => {
  const raw = response.headers.get("X-Consolidado-Summary");
  if (!raw) return null;
  try {
    return JSON.parse(decodeURIComponent(raw)) as ConsolidadoSummary;
  } catch (_error) {
    return null;
  }
};

const renderSummary = (summary: ConsolidadoSummary | null): void => {
  if (!summary) {
    consolidadoSummaryPanel.hidden = true;
    return;
  }
  Object.entries(summary).forEach(([key, value]) => {
    const target = consolidadoSummaryPanel.querySelector<HTMLElement>(`[data-summary='${key}']`);
    if (target) target.textContent = String(value);
  });
  consolidadoSummaryPanel.hidden = false;
};

const validateConsolidadoFiles = (): boolean => {
...
consolidadoForm.addEventListener("submit", async (event) => {
  ...
  renderSummary(null);
  resetConsolidadoDownload();
  if (!validateConsolidadoFiles()) return;
  ...
  try {
    ...
    if (!response.ok) {
      ...
      let data: ApiError = {};
      try {
        data = JSON.parse(errorBody) as ApiError;
      } catch (_error) {
        // Some server/proxy errors are returned as HTML instead of JSON.
      }
...
consolidadoDownloadButton.addEventListener("click", () => {
  if (consolidadoBlob) downloadBlob(consolidadoBlob, consolidadoFilename);
});
